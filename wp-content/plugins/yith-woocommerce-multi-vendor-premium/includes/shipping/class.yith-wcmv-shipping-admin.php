...
if ( !class_exists( 'YITH_Vendor_Shipping_Admin' ) ) {
    ...
    class YITH_Vendor_Shipping_Admin {
        ...
        public function enqueue_scripts() {
            ...
            wp_register_script( $handle, YITH_WPV_ASSETS_URL . 'js/shipping.js', $deps, YITH_WPV_VERSION, true );
        ...
        public function print_line_option( $index, $zone , $continents = null , $allowed_countries = null , $shipping_methods = null ) {
            ...
            yith_wcpv_get_template( 'vendor-admin-shipping-table-row',array( 'index' => $index , 'zone' => $zone , 'continents' => $continents , 'allowed_countries' => $allowed_countries , 'shipping_methods' => $shipping_methods ), 'admin/vendor-panel' );
        ...
        public function yith_wpv_shipping_add_new_option() {
            ...
            $this->print_line_option( uniqid(), null, $continents, $allowed_countries );
        ...
        private function print_single_method_in_cell( $wc_method , $area_key , $index , $method) {
            ...
            $title = isset($method['method_title']) ? $method['method_title'] : $wc_method->method_title;
            $tax_status = isset($method['method_tax_status']) ? $method['method_tax_status'] : '';
            $cost = isset($method['method_cost']) ? $method['method_cost'] : '';
            $method_requires = isset($method['method_requires']) ? $method['method_requires'] : '';
            $min_amount = isset($method['min_amount']) ? $method['min_amount'] : '';

            $method_id = isset( $method['type_id'] ) ? $method['type_id'] : $wc_method->id;
            ...
            echo '<input type="hidden" id="method_title_'.$index.'" class="wc-shipping-zone-method-hidden-data" name="yith_vendor_data[zone_data][' . esc_attr( $area_key ) . '][zone_shipping_methods][' . esc_attr( $index ) . '][method_title]" value="' . esc_attr( $title ) . '" />';
            echo '<input type="hidden" id="method_tax_status_'.$index.'" class="wc-shipping-zone-method-hidden-data" name="yith_vendor_data[zone_data][' . esc_attr( $area_key ) . '][zone_shipping_methods][' . esc_attr( $index ) . '][method_tax_status]" value="' . esc_attr( $tax_status ) . '" />';
            echo '<input type="hidden" id="method_cost_'.$index.'" class="wc-shipping-zone-method-hidden-data" name="yith_vendor_data[zone_data][' . esc_attr( $area_key ) . '][zone_shipping_methods][' . esc_attr( $index ) . '][method_cost]" value="' . esc_attr( $cost ) . '" />';

            echo '<input type="hidden" id="method_requires_'.$index.'" class="wc-shipping-zone-method-hidden-data" name="yith_vendor_data[zone_data][' . esc_attr( $area_key ) . '][zone_shipping_methods][' . esc_attr( $index ) . '][method_requires]" value="' . esc_attr( $method_requires ) . '" />';
            echo '<input type="hidden" id="min_amount_'.$index.'" class="wc-shipping-zone-method-hidden-data" name="yith_vendor_data[zone_data][' . esc_attr( $area_key ) . '][zone_shipping_methods][' . esc_attr( $index ) . '][min_amount]" value="' . esc_attr( $min_amount ) . '" />';
