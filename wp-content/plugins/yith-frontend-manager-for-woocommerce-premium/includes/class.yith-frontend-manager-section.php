...
if ( ! class_exists( 'YITH_Frontend_Manager_Section' ) ) {
	...
	class YITH_Frontend_Manager_Section {
		...
		public function is_current( $subsection = '' ) {
			...
			if ( $subsection && $is_current ) {
				$is_current = $subsection == $wp->query_vars[ $this->slug ];
			}
		...
		public function get_name( $subsection = '' ) {
			if( $subsection ){
				$subsection_name  = $this->get_option( 'name', $this->id . '_' . $subsection, $subsection );
			}

			return isset( $subsection_name ) ? $subsection_name : $this->_name;
		}
		...
		final public function enqueue_scripts(){
            ...
            if( $is_current || $is_default_section ){
        ...
        public function body_classes( $classes ){
            if( YITH_Frontend_Manager()->gui->is_main_page() || $this->is_current() ){
               ...
               if( is_child_theme() ){
                   ...
                   $parent_theme_name = strtolower( $wp_theme->Template );
...
add_action( 'yith_wcfm_after_load_common_classes', 'YITH_Frontend_Manager_Section::section_class_ î     î 